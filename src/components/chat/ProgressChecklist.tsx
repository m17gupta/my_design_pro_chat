...
export default function ProgressChecklist({
  ...
  console.log("ProgressChecklist componet----->")
  ...
  const doneCount = checklist.filter((item) =>
    completed.has(item.id)
  ).length;

  const activeChecklistId = currentId
    ? episodes?.find((episode) => episode.apiKey === currentId)
        ?.checklistId ?? null
    : null;
    ...
    <div>
      ...
      <ol className="space-y-1">
        {visibleItems.map(({ item, displayText, isTyping }) => {
          ...
          const isActive =
            activeChecklistId === item.id && !isDone;
